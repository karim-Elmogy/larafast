...
use Elmogy\Larafast\Logic\Validations\UnitValidation;
use Elmogy\Larafast\Processors\Factory;
...
class MakeUnit extends Base
{
    ...
    protected function publishUnit()
    {
        if($this->init) {
            ...
            $this->command->info(__('larafast::unit.success_init_not_executed'));
        }
    }
    ...
    private function publishUnitInitCase() {
        ...
        $files             = config('larafast.module.unit_main_folders');
        ...
        $this->command->info(__('larafast::unit.success_init_executed'));
    }
}
