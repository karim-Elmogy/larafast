...
class MakeModule extends Base
{
    ...
    public function run()
    {
        ...
        if (!FileManipulator::exists($this->selected_dir)) {
            $allow_publish = false;
            $this->command->error(__('larafast::module.run_install'));
        }

        if (FileManipulator::exists($module_dir)) {
            if ($this->command->confirm(__('larafast::module.exists'), true)) {
                $allow_publish = true;
            } else {
                $allow_publish = false;
                $this->command->warn(__('larafast::module.not_overwritten'));
            }
        }
    ...
    protected function publishModuleDirectory($module_collection)
    {
        ...
        $files           = config('larafast.module.main_files');
        ...
        $this->command->info(__('larafast::module.success'));
    }
}
