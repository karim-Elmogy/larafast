...
namespace Elmogy\Larafast\Logic\Validations;

use Illuminate\Console\Command;
use Elmogy\Larafast\Logic\FileManipulator;
use Elmogy\Larafast\Logic\StringManipulator;
...
class UnitValidation
{
    ...
    protected function checkModule($module_dir)
    {
        if (!FileManipulator::exists($module_dir)) {
            ...
            $this->command->error(__('larafast::unit.module_missing'));
        }
    }
    ...
    protected function checkUnit($module_dir, $unit_collection, $init)
    {
        if ($init) {
            if (FileManipulator::exists("{$module_dir}/data/{$unit_collection['studly']}.json")) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.init_executed'));
            }
        } else {
            if (!FileManipulator::exists("{$module_dir}/data/{$unit_collection['studly']}.json")) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.init_not_executed'));
            }
            ...
            if (FileManipulator::exists("{$module_dir}/Requests/{$unit_collection['studly']}Request.php")) {
                if ($this->command->confirm(__('larafast::unit.exists'), true)) {
                    $this->allow_proceed = true;
                } else {
                    $this->allow_proceed = false;
                    $this->command->warn(__('larafast::unit.not_overwritten'));
                }
            }
        }
    }
    ...
    protected function checkAttributes($root_dir, $module_collection, $unit_collection) {
        ...
        if (FileManipulator::exists($full_path)) {
            ...
            if ($this->attributes == null) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.attributes_prop_required'));
            } else {
                foreach ($this->attributes as $column_name => $cases) {
                    ...
                    if(!isset($cases['type'])) {
                        ...
                        $this->command->error(__('larafast::unit.type_prop_required', $params));
                    } else {
    ...
    protected function typeCase($type_value, $column, $unit_collection) {
        ...
        if(!$this->isSchemaFound('types', $type)) {
            $this->allow_proceed = false;
            $this->command->error(__('larafast::unit.type_prop_not_valid', ['type' => $type]));
    ...
    protected function handleTypeValue($schema_types, $type_value) {
        ...
        if($has_value) {
            ...
            if(!$this->hasValue($values)) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.type_prop_has_value', ['type' => $type]));
            }
        } else {
            // ====== (5) ====
            if($this->hasValue($values)) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.type_prop_has_no_value', ['type' => $type]));
            }
        }
    }
    ...
    protected function definitionCase($definition_value) {
        ...
        foreach ($definitions as $single_definition) {
            ...
            if(!$this->isSchemaFound('definitions', $definition)) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.definition_prop_not_valid', ['definition' => $definition]));
            } else {
    ...
    protected function handleDefinitionValue($schema_definitions, $definition_value) {
        ...
        if($has_value) {
            ...
            if(!$this->hasValue($values)) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.definition_prop_has_value', ['definition' => $definition]));
            }
        } else {
            // ====== (8) ====
            if($this->hasValue($values)) {
                $this->allow_proceed = false;
                $this->command->error(__('larafast::unit.definition_prop_has_no_value', ['definition' => $definition]));
            }
        }
    }
